...
int AbstractGame::runMainLoop() {
	...
	while (running) {
		...
		if (!paused) {
			...
			renderUI();
			// Update UI
			gameUIController.updateUI(eventSystem); 
			gameUIController.drawUI(gfx);
			gfx->showScreen();
		}
		else
		{
			...
			if (easterEgg) {
				gfx->drawUIElementText("Beck Farrington-Wheeler - CI517 - 2023", DEFAULT_WINDOW_WIDTH / 3, 25); 
			}

			// Update UI
			menuUIController.updateUI(eventSystem);
			menuUIController.drawUI(gfx);
			gfx->showScreen();
		}
...
void AbstractGame::testFunction() {
	//printf("Test function has been called...");
}
...
void AbstractGame::changeEasterEgg() {
	easterEgg = !easterEgg; 
}
