...
void GraphicsEngine::useUIFont(TTF_Font* _uiFont) {
	if (nullptr == _uiFont) {
#ifdef __DEBUG
		debug("GraphicsEngine::useFont()", "font is null");
#endif
...
void GraphicsEngine::drawUIElementText(const std::string& text, const int& x, const int& y) {
	SDL_Texture* textTexture = createTextureFromString(text, uiFont, drawColor);
	int w, h;
	SDL_QueryTexture(textTexture, 0, 0, &w, &h);
	SDL_Rect dst = { x, y, w, h };
	drawTexture(textTexture, &dst);
